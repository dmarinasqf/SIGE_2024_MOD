...
var txtidlaboratorio = $('#txtidlaboratorio');
var hdidlaboratorio = $('#hdidlaboratorio');
...
function limpiar() {
    ...
    txtcodigo.val(hdidlaboratorio.val());
}

$('#form-registro').submit(function (e) {
    e.preventDefault();
    var url = ORIGEN + "/Almacen/ALaboratorio/RegistrarEditarRepresentante";
...
$(document).on('click', '.btn-pasar', function (e) {
    ...
    txtdocumento.val(columna[1]);
    txtnombres.val(columna[2]);

    txttelefono.val(columna[3]);
    txtcelular.val(columna[4]);
    txtemail.val(columna[5]);
    cmbestado.val(columna[6]);
...
function agregarFila(data) {
    ...
    tbllista.row.add([
        ...
        `<div class="btn-group btn-group-sm" >            
            ...
            <button class="btn btn-sm bg-danger waves-effect text-white" onclick="mensajeeliminar(`+ data.idrepresentante + `)"><i class="fas fa-trash-alt"></i></button>                                                                 
        </div>`
    ]).draw(false);
}
...
function eliminar(id) {
    var url = ORIGEN + "/Compras/CLaboratorio/EliminarRepresentante/" + id;

...
$('#form-busqueda').submit(function (e) {
    ...
    controller.ListarGuiasEntrada(obj, listarGuiaEntrada);
});
        ...
        if (data.mensaje === "ok") {
            ...
            for (let i = 0; i < datos.length; i++) {
                let botones = `<div class="btn-group btn-group-sm" >            
                            <a class="btn btn-sm btn-outline-primary waves-effect btn-pasar font-10" data-toggle="tooltip" data-placement="top" title="HACER PICKING" href="`+ ORIGEN + `/Almacen/AGuiaEntrada/Registrar/?id=` + datos[i]["ID"] + `"><i class="fa fa-eye"></i></a>
                        </div>`;
...
function fnGetDetalleGuiaSalida(id) {
    ...
    if (index === -1)
        ...
        for (var i = 0; i < detalle.length; i++) {
            ...
            tbody += '<td>' + detalle[i]['CANTIDAD ENVIADA'] + '</td>';
            tbody += '<td>' + detalle[i]['CANTIDAD INGRESO'] + '</td>';

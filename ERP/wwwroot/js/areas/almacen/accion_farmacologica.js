...
$('#form-registro').submit(function (e) {
    e.preventDefault();
    var url = ORIGEN + "/Almacen/AAccionFarmacologica/RegistrarEditar";
...
function agregarFila(data) {
    ...
    if (data.estado === "HABILITADO") {
        tbllista.row.add([
            ...
            <button class="btn btn-sm btn-outline-danger waves-effect  font-10" onclick="mensajeeliminar(`+ data.idaccionfarma + `)"><i class="fa fa-lock"></i></button>                                              
        </div>`
        ]).draw(false);
    } else {
        tbllista.row.add([
            ...
            data.estado,
            `<div class="btn-group btn-group-sm" >   
             <button class="btn btn-sm btn-outline-success waves-effect font-10" onclick="habilitar(`+ data.idaccionfarma + `)"><i class="fa fa-unlock"></i></button>                                           
        </div>`
        ]).draw(false);
    }
}
...
function eliminar(id) {
    var url = ORIGEN + "/Almacen/AAccionFarmacologica/Eliminar/" + id;
...
function habilitar(id) {
    var url = ORIGEN + "/Almacen/AAccionFarmacologica/Habilitar/" + id;
    $.post(url).done(function (data) {
        if (data.mensaje === "ok") {
            {
                tbllista.row('.selected').remove().draw(false);

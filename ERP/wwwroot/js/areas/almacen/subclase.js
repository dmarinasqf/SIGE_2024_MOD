...
$('#form-registro').submit(function (e) {
    e.preventDefault();
    var url = ORIGEN + "/Almacen/ASubClase/RegistrarEditar";
    ...
    $.post(url, obj).done(function (data) {
        if (data.mensaje === "ok") {
            ...
            data.objeto.clase = $('#cmbclase option:selected').text();
...
function agregarFila(data) {
    ...
    tbllista.row.add([
        ...
        `<div class="btn-group btn-group-sm" >            
            ...
            <button class="btn btn-sm bg-danger waves-effect text-white" onclick="mensajeeliminar(`+ data.idsubclase + `)"><i class="fas fa-trash-alt"></i></button>                                                                 
        </div>`
    ]).draw(false);
}
...
function eliminar(id) {
    var url = ORIGEN + "/Almacen/ASubClase/Eliminar/" + id;

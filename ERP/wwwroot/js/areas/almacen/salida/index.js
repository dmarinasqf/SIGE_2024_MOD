...
function fnGetHistorial() {
    ...
    controller.GetHistorial(obj, function (data) {
        ...
        for (var i = 0; i < data.length; i++) {
            btnimp = '<button class="btn btn-dark btnimprimir"  onclick="fnimprimir(' + data[i].idsalida+')"><i class="fas fa-print" ></i></button>';
           var fila= tbllista.row.add([
            ...
            fila.getElementsByTagName('td')[0].setAttribute('idsalida', data[i].idsalida);
        }
...
function fnagregardetalleatabla(idsalida) {
    var index = fnencontrarindexarray(idsalida);
    if (index == -1) {
...
function fnencontrarindexarray(idsalida) {
  
    for (var i = 0; i < _DETALLE.length; i++) {
        if (_DETALLE[i].idsalida == idsalida)
            return i;
    }
    return -1;
}
...
$('#tbllista tbody').on('click', 'tr td.details-control', function () {
    ...
    if (row.child.isShown()) {
        ...
        tr.addClass('details');
        row.child(fnagregardetalleatabla(this.getAttribute('idsalida'))).show();
    }
});


function fnimprimir(idsalida) {
    var href =   ORIGEN + '/Almacen/ASalida/Imprimir_Guia/?idsalida='+idsalida;

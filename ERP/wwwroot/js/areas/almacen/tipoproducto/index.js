...
$('#form-registro').submit(function (e) {
    e.preventDefault();
    var url = ORIGEN + "/Almacen/ATipoProducto/RegistrarEditar";
...
function agregarFila(data) {
    let botones = data.estado === "HABILITADO" ? `<div class="btn-group btn-group-sm" >            
            ...
            <button class="btn btn-sm btn-outline-danger waves-effect  font-10" onclick="mensajeeliminar(`+ data.idtipoproducto + `)"><i class="fa fa-lock"></i></button>                                              
        </div>`: `<div class="btn-group btn-group-sm" >   
             <button class="btn btn-sm btn-outline-success waves-effect font-10" onclick="habilitar(`+ data.idtipoproducto + `)"><i class="fa fa-unlock"></i></button>                                           
        </div>`;
...
function eliminar(id) {
    var url = ORIGEN + "/Almacen/ATipoProducto/Eliminar/" + id;
...
function habilitar(id) {
    var url = ORIGEN + "/Almacen/ATipoProducto/Habilitar/" + id;
    $.post(url).done(function (data) {
        if (data.mensaje === "ok") {
            {
                tbllista.row('.selected').remove().draw(false);

...
$('#form-registro').submit(function (e) {
   ...
   // alert(txtoperacion.val());
    var url = ORIGEN + "/Almacen/AAreaAlmacen/RegistrarEditar";
...
function agregarFila(data) {
    ...
    if (data.estado === "HABILITADO") {
        tbllista.row.add([
            ...
            <button class="btn btn-sm btn-outline-danger waves-effect  font-10" onclick="mensajeeliminar(`+ data.idareaalmacen + `)"><i class="fa fa-lock"></i></button>                                                                   
        </div>`
        ]).draw(false);
    }
    else {
        tbllista.row.add([
            ...
            data.estado,
            `<div class="btn-group btn-group-sm" >        
             <button class="btn btn-sm btn-outline-success waves-effect  font-10" onclick="habilitar(`+ data.idareaalmacen + `)"><i class="fa fa-unlock"></i></button>                                                                   
        </div>`
        ]).draw(false);
    }
}
...
function eliminar(id) {
    var url = ORIGEN + "/Almacen/AAreaAlmacen/Eliminar/" + id;
...
function habilitar(id) {
    var url = ORIGEN + "/Almacen/AAreaAlmacen/Habilitar/" + id;
    $.post(url).done(function (data) {
        if (data.mensaje === "ok") {
            {
                tbllista.row('.selected').remove().draw(false);

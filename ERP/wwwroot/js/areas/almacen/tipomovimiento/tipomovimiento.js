...
$('#form-registro').submit(function (e) {
    e.preventDefault();
    var url = ORIGEN + "/Almacen/ATipoMovimiento/RegistrarEditar";
...
function agregarFila(data) {
    console.log(data);
    let botones = data.estado === 'HABILITADO' ? `<div class="btn-group btn-group-sm" >            
            <a class="btn btn-sm btn-outline-warning waves-effect btn-pasar font-10"><i class="fa fa-edit"></i></a>
            <button class="btn btn-sm btn-outline-danger waves-effect  font-10" onclick="mensajeeliminar(`+ data.idtipomovimiento + `)"><i class="fa fa-lock"></i></button>                                              
        </div>` : `<div class="btn-group btn-group-sm" >   
             <button class="btn btn-sm btn-outline-success waves-effect font-10" onclick="habilitar(`+ data.idtipomovimiento + `)"><i class="fa fa-unlock"></i></button>                                           
        </div>`;   
        tbllista.row.add([
...
function eliminar(id) {
    var url = ORIGEN + "/Almacen/ATipoMovimiento/Eliminar/" + id;
...
function habilitar(id) {
    var url = ORIGEN + "/Almacen/ATipoMovimiento/Habilitar/" + id;
    $.post(url).done(function (data) {
        if (data.mensaje === "ok") {
            {
                tbllista.row('.selected').remove().draw(false);

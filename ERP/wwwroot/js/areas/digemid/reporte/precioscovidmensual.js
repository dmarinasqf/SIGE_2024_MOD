...
btnconsultar.addEventListener('click', function () {
    ...
    controller.GetPreciosCovid(obj, function (data) {

        if (data.length > 0) {
           
            for (var i = 0; i < data.length; i++) {
                if (data[i]['Preciov'] != undefined)data[i]['Preciov'] = data[i]['Preciov'].toFixed(2);
                if (data[i]['PrecioMin'] != undefined) data[i]['PrecioMin'] = data[i]['PrecioMin'].toFixed(2);
                if (data[i]['PrecioMax'] != undefined) data[i]['PrecioMax'] = data[i]['PrecioMax'].toFixed(2);
                if (data[i]['PrecioMed'] != undefined) data[i]['PrecioMed'] = data[i]['PrecioMed'].toFixed(2);
            }
...
btnexportar.addEventListener('click', function () {
    ...
    let controller = new ReporteController();
    controller.ExcelPreciosCovid(obj);
});
...
$(document).on('click', '.btnverproducto', function () {
    ...
    console.log(idanterior, '->', idproducto);
    if (idproducto != '') {
        if (idanterior == '') {

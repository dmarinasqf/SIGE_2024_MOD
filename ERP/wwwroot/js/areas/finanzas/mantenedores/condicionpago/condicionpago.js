...
$('#form-registro').submit(function (e) {
   ...
   // alert(txtoperacion.val());
    var url = ORIGEN + "/Finanzas/FCondicionPago/RegistrarEditar";
...
function agregarFila(data) {
    console.log(data);
    tbllista.row.add([
        data.idcondicionpago,
        data.descripcion,          
        data.ndias,          
        data.estado,
        `<div class="btn-group btn-group-sm" >            
            <button class="btn btn-sm bg-warning waves-effect btn-pasar"><i class="fas fa-edit"></i></button>
            <button class="btn btn-sm bg-danger waves-effect text-white" onclick="mensajeeliminar(`+ data.idalmacen + `)"><i class="fas fa-trash-alt"></i></button>                                                                 
        </div>`
    ]).draw(false);
}
...
function eliminar(id) {
    var url = ORIGEN + "/Finanzas/FCondicionPago/Eliminar/" + id;

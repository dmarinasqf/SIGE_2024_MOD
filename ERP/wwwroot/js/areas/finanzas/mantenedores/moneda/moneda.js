...
$('#form-registro').submit(function (e) {
    e.preventDefault();
    var url = ORIGEN + "/Finanzas/FMoneda/RegistrarEditar";
...
$(document).on('click', '.btn-pasar', function (e) {
    ...
    txtcodigosunat.val(columna[2]);
    txtsimbolo.val(columna[3]);
    txttipocambio.val(columna[4]);
...
function agregarFila(data) {
    ...
    tbllista.row.add([
        ...
        data.codigosunat,
        data.simbolo,
        parseFloat(data.tipodecambio),
        data.estado,
        `<div class="btn-group btn-group-sm" >            
            <button class="btn btn-sm bg-warning waves-effect btn-pasar"><i class="fas fa-edit"></i></button>
            <button class="btn btn-sm bg-danger waves-effect text-white" onclick="mensajeeliminar(`+ data.idmoneda + `)"><i class="fas fa-trash-alt"></i></button>                                                                 
        </div>`
    ]).draw(false);
}
...
function eliminar(id) {
    var url = ORIGEN + "/Finanzas/FMoneda/Eliminar/" + id;

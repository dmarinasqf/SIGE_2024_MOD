...
function fnListarPedidos() {
    ...
    controller.ListarPedidoPorUsuario('RECEPCIONADO', fnLLenarTablaPedidos);
}
...
$(document).on('click', '.checkasignar', function (response) {
    ...
    if (this.checked)
        controller.CambiarEstadoEntregaDelivery('EN RUTA', id, function () {
            mensaje('S', 'Pedido En Ruta');
        });
    else

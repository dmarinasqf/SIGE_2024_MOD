...
{
    public class GetReporteTemperatura
    {
        ...
        public class Manjeador : IRequestHandler<Ejecutar, object>
        {
            private readonly IEjecutarProcedimiento procedimiento;

            public Manjeador(IEjecutarProcedimiento procedimiento)
            {
            ...
            {
                var stroreprocedure = "asistencia.SP_ReporteTemperatura";
                ...
                if (e.top > 1000)
                {
                    var tabla = await procedimiento.HandlerDatatableAsync(stroreprocedure, parametros, "ReporteTemperatura");
            ...
            public async Task<mensajeJson> guardarExcel(string path, DataTable tabla)
            {
                try
                {
                    var data = await Task.Run(() =>
                    {
                        ...
                        var nombre = "ReporteTemperatura" + DateTime.Now.ToString("yyyyMMddHHmm") + ".xlsx";

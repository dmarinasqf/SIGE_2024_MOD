...
{
    public class ReporteFormulasMagistrales
    {
        ...
        public class Manejador : IRequestHandler<Ejecutar, object>
        {
            ...
            {
                var stroreprocedure = "SP_REPORTEFORMULASMAGISTRALES";
                ...
                parametros.Add("PERFIL", request.perfil ?? "");
                parametros.Add("LABORATORIO", request.laboratorio ?? "");
                parametros.Add("HORAINICIO", request.horainicio ?? "");
                parametros.Add("HORAFIN", request.horafin ?? "");
                parametros.Add("TIPOEMPRESA", request.tipoempresa ?? "");
                parametros.Add("TIPOPRODUCTO", request.tipoproducto ?? "");
                parametros.Add("CONSULTA", request.tipo ?? "CONSULTA");
               
                if (request.tipo == "EXPORTACION")
                {
                    var tabla = await procedimiento.HandlerDatatableAsync(stroreprocedure, parametros, "FormulasMagistrales");
            ...
            public async Task<mensajeJson> guardarExcel(string path, DataTable tabla)
            {
                try
                {
                    var data = await Task.Run(() =>
                    {
                        ...
                        var nombre = "ReporteFormulasMagistrales" + DateTime.Now.ToString("yyyyMMddHHmm") + ".xlsx";

...
{
    public class ListarDepositosAprobar
    {
        ...
        public class Manejador : IRequestHandler<Ejecutar, object>
        {
            ...
            public async Task<object> Handle(Ejecutar e, CancellationToken cancellationToken)
            {
                ...
                if (e.top > 1000)
                {
                    var tabla = await procedimiento.HandlerDatatableAsync(stroreprocedure, parametros, "ListarDepositoAprobado");
            ...
            public async Task<mensajeJson> guardarExcel(string path, DataTable tabla)
            {
                try
                {
                    var data = await Task.Run(() =>
                    {
                        ...
                        string direccion = "/archivos/reportes/deposito/";
